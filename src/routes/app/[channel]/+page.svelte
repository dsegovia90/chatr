	...
	<div bind:this={scroller} class="flex-grow p-6 overflow-y-auto">
		<div class="flex justify-end flex-col">
			{#each $messages as [id, message] (id)}
				<div class="flex mx {message.createdBy.id === $user.id ? 'justify-start' : 'justify-end'}">
					<div
						class="px-4 py-2 my-2 rounded {message.createdBy.id === $user.id
							? 'bg-blue-300'
							: 'bg-green-300'}"
					>
						{message.text}
	...
	<div class="p-4 border-t bg-slate-200">
		<form on:submit|preventDefault={handleSendMessage} class="flex">
			<input
				...
				placeholder=""
			/>
			<button type="submit" class="ml-2 px-8 py-2 bg-green-400 rounded">Send</button>
		</form>
	</div>
</div>
